...
`simple-snowplow` is a lightweight self-hosted analytics collector compatible with the Snowplow protocol.

Use it to:
- receive Snowplow-style tracking requests over HTTP;
...
- prefer `uv run python simple_snowplow/cli.py ...` from the repo root;
...
- demo routes exist only when `SNOWPLOW_COMMON__DEMO=true`;
...
- `uv run python simple_snowplow/cli.py db init`
- `uv run python simple_snowplow/cli.py queue worker`
- `uv run python simple_snowplow/cli.py queue healthcheck`
...
- [simple_snowplow](simple_snowplow): application package.

Important entrypoints:
- [simple_snowplow/main.py](simple_snowplow/main.py): FastAPI app factory and router wiring.
- [simple_snowplow/cli.py](simple_snowplow/cli.py): operational CLI for settings, ClickHouse init, queue worker, and script download.
- [simple_snowplow/core/config.py](simple_snowplow/core/config.py): full env-driven settings model.
- [simple_snowplow/core/healthcheck.py](simple_snowplow/core/healthcheck.py): probe behavior.

Main package areas:
- `simple_snowplow/core`: config, lifespan, healthcheck, shared plumbing.
- `simple_snowplow/routers/tracker`: tracker routes, parsers, models, ClickHouse integration.
- `simple_snowplow/routers/proxy`: proxy functionality.
- `simple_snowplow/routers/demo`: demo assets and routes.
- `simple_snowplow/ingest`: RabbitMQ-backed worker path.
- `simple_snowplow/plugins`: logging and optional integrations.

## Project-Specific Guardrails

Do not casually "fix" the import style across the repo.
- this codebase intentionally supports both `simple_snowplow...` imports and app-root imports such as `from core...`;
...
- preserve the `SNOWPLOW_` env prefix and nested `__` structure;
